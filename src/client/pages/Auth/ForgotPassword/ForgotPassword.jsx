...
const ForgotPassword = ({ initialEmail }) => {
  const [email, setEmail] = React.useState(initialEmail || ""); // Thêm state để lưu email
  ...
  const [formState, setFormState] = React.useState("enterEmail"); // Trạng thái ban đầu là nhập email
  ...
  const sendEmail = async () => {
    try {
      ...
      // Tìm button có id `openModalOTP` và tự động click
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Hiển thị input nhập mật khẩu mới
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("New Password:", formData.newPassword); // Debug newPassword
                    ...
                    {formState === "enterEmail" && (
                      <>
                        ...
                        <InputFlied
                          ...
                          content={"Email"}
                          dai={400}
                          onChange={(e) => setEmail(e.target.value)} // Lưu email khi nhập
                        />
                        <div
